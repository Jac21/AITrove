...
class RagArchitectureAgent(Agent):
    ...
    @property
    def name(self) -> str:
        return "RagArchitectureAgent"

    def can_handle(self, ctx: AgentContext) -> bool:
        return "rag_brief" in ctx.metadata

    async def execute(self, ctx: AgentContext) -> str:
        system_prompt = await self._prompts.load("rag-architecture.system")
        user_template = await self._prompts.load("rag-architecture.user")
        user_message = (
            user_template
