...
class RetrievalStrategyAgent(Agent):
    ...
    async def execute(self, ctx: AgentContext) -> str:
        system_prompt = await self._prompts.load("retrieval-strategy.system")
        user_template = await self._prompts.load("retrieval-strategy.user")
        user_message = (
            user_template
